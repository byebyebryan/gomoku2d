...
pub const ANALYSIS_FIXTURE_CASES: &[AnalysisFixtureCase] = &[
    ...
    AnalysisFixtureCase {
        ...
        expected: AnalysisFixtureExpected {
            ...
            critical_loser_ply: Some(10),
            tactical_notes: &[],
            failure_mode: Some(FailureMode::MissedLethalPrevention),
            failure_prefix_ply: Some(9),
            failure_candidates: &["K8"],
            required_unknown_gaps: &[],
...
fn fixture_failures(
    ...
    if actual.critical_loser_ply != expected.critical_loser_ply {
        failures.push(format!(
            "critical_loser_ply expected {:?}, got {:?}",
            expected.critical_loser_ply, actual.critical_loser_ply
        ));
    }
...
mod tests {
    ...
    use super::{run_analysis_fixtures, ANALYSIS_FIXTURE_CASES};
    use crate::analysis::{
