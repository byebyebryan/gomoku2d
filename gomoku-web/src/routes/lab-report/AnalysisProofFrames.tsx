...
import proofStyles from "./AnalysisProofFrames.module.css";

const DEFAULT_REPORT_BOARD_SIZE = 15;
const ANALYSIS_BOARD_CELL_SIZE = 20;
const ANALYSIS_BOARD_LABEL_SIZE = 16;

export function ProofFrames({ entry }: { entry: AnalysisEntry }) {
  ...
  return (
    <div className={proofStyles.frames}>
      <ProofLegend />
      <ProofFrameArticle
        boardSize={boardSize}
        frame={winningFrame}
...
function ProofLegend() {
  return (
    <div className={proofStyles.proofLegend} aria-label="Proof marker legend">
      <div className={proofStyles.proofLegendRow}>
        <LegendRole className={proofStyles.legendWinning} label="immediate win" />
        <LegendRole className={proofStyles.legendThreat} label="immediate threat" />
        <LegendRole className={proofStyles.legendImminent} label="imminent threat" />
        <LegendRole className={proofStyles.legendOffensive} label="counter threat" />
        <LegendRole className={proofStyles.legendCorridorEntry} label="corridor entry" />
      </div>
      <div className={proofStyles.proofLegendRow}>
        <LegendOutcome className={proofStyles.legendImmediateLoss} marker="!" label="immediate loss" />
        <LegendOutcome className={proofStyles.legendForced} marker="L" label="forced loss" />
        <LegendOutcome className={proofStyles.legendForbidden} marker="X" label="forbidden" />
        <LegendOutcome className={proofStyles.legendConfirmed} marker="E" label="confirmed escape" />
        <LegendOutcome className={proofStyles.legendPossible} marker="P" label="possible escape" />
        <LegendOutcome className={proofStyles.legendUnknown} marker="?" label="unknown" />
      </div>
    </div>
  );
}
...
function LegendRole({ className, label }: { className: string; label: string }) {
  return <span className={`${proofStyles.legendRole} ${className}`}>{label}</span>;
}

function LegendOutcome({ className, marker, label }: { className: string; marker: string; label: string }) {
  return (
    <span className={proofStyles.legendOutcome}>
...
function ProofFrameArticle({
  ...
  return (
    <article className={`${proofStyles.frame} ${proofStyles.analysisFrame}`} data-ply={frame.ply}>
      <ProofBoard
        boardSize={boardSize}
        ...
        moveCells={moveCells}
      />
      <div className={proofStyles.proofFrameCopy}>
        <h3>{title}</h3>
        <div className={proofStyles.proofFrameLines}>
          {lines.map(([label, value]) => (
            <div className={proofStyles.proofFrameLine} key={label}>
              <span>{label}</span>
              <strong>{value}</strong>
            </div>
          ))}
...
function ReplyOutcomes({ frame }: { frame: ProofFrame }) {
  ...
  return (
    <div className={proofStyles.replyOutcomes}>
      <div className={`${proofStyles.replyOutcomeRow} ${proofStyles.replyOutcomeHeader}`}>
        <span>Move</span>
        <span>Role</span>
        <span>Outcome</span>
      </div>
      {frame.reply_outcomes.map((reply) => (
        <div className={proofStyles.replyOutcomeRow} key={`${reply.notation}-${reply.outcome}`}>
          ...
          {Array.from({ length: boardSize }, (_, col) => {
            ...
            return (
              ...
              >
                ...
                {actualSide ? (
                  <span
                    className={`${proofStyles.proofActualStone} ${
                      actualSide === "Black"
                        ? proofStyles.proofActualStoneBlack
                        : proofStyles.proofActualStoneWhite
                    }`}
                    aria-label={`${actualSide} actual move at ${cellNotation(cell, boardSize)}`}
                  />
                ) : null}
                {label ? <span className={proofStyles.proofMarker}>{label}</span> : null}
              </div>
            );
          })}
          <div className={`${styles.proofCoordinate} ${styles.proofCorner}`} aria-hidden="true" />
...
function markerKindClass(kind: string): string | null {
  switch (kind) {
    case "winning":
      return proofStyles.markerWinning;
    case "threat":
      return proofStyles.markerThreat;
    case "imminent_defense":
      return proofStyles.markerImminentDefense;
    case "offensive_counter":
      return proofStyles.markerOffensiveCounter;
    case "winning_evidence":
      return proofStyles.markerWinningEvidence;
    case "threat_evidence":
      return proofStyles.markerThreatEvidence;
    case "imminent_evidence":
      return proofStyles.markerImminentEvidence;
    case "offensive_evidence":
      return proofStyles.markerOffensiveEvidence;
    case "corridor_entry_black":
      return proofStyles.markerCorridorEntryBlack;
    case "corridor_entry_white":
      return proofStyles.markerCorridorEntryWhite;
    case "forbidden":
      return proofStyles.markerForbidden;
    case "forced_loss":
      return proofStyles.markerForcedLoss;
    case "immediate_loss":
      return proofStyles.markerImmediateLoss;
    case "confirmed_escape":
      return proofStyles.markerConfirmedEscape;
    case "possible_escape":
      return proofStyles.markerPossibleEscape;
    case "unknown_outcome":
      return proofStyles.markerUnknownOutcome;
    default:
      return null;
  }
}
